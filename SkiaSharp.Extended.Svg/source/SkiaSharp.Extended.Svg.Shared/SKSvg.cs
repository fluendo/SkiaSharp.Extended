	...
	public class SKSvg
	{
		...
		private void ReadElement(XElement e, SKCanvas canvas, SKPaint stroke, SKPaint fill)
		{
			...
			switch (elementName)
			{
				case "style":
					{
						styles = CssHelper.ParseSelectors(e.Value);
						break;
					}
				case "image":
					{
		...
		private Dictionary<string, string> ReadStyle(XElement e)
		{
			...
			if (styles != null && styles.TryGetValue(e.Name.LocalName, out string glStyle))
			{
			...
			if (styles != null &&
				dic.TryGetValue("class", out string className) &&
